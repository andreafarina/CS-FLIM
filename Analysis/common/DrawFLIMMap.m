function [] = DrawFLIMMap(lifetime,concentration,N,residuals)

% Plot the results
for li = 1:size(lifetime,1)
    for n = 1:N
        figure(li), subplot(1,N+1,n),
    ...
    imagesc(squeeze(residuals(li,:,:)));axis image,colormap(resplot,'parula'),colorbar
    title('Residuals')
    end
end
...
for li=1:size(lifetime,1)
    figure(size(lifetime,1)+li)
    for n = 1:N
        subplot(1,N,n)
        taus = squeeze(lifetime(li,:,:,n));
        histogram(taus(taus>0),round(numel(lifetime(li,:,:,n))/30))
    end
end
